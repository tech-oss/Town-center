  ...
  return (
    <div className="bg-white rounded-2xl p-6 flex flex-col gap-4" style={{ boxShadow: "0 2px 12px rgba(13,42,51,0.07)", border: "1px solid rgba(10,25,47,0.12)" }}>
      ...
      <label className="flex flex-col gap-1">
        ...
        <input value={form.title} onChange={(e) => set("title", e.target.value)} className="border rounded-xl px-3 py-2 text-sm outline-none" style={{ border: "1.5px solid rgba(10,25,47,0.2)", color: "#0A192F" }} />
      </label>
      <label className="flex flex-col gap-1">
        <span className="text-xs font-semibold" style={{ color: "#6B7280" }}>Image URL</span>
        <input value={form.image} onChange={(e) => set("image", e.target.value)} className="border rounded-xl px-3 py-2 text-sm outline-none" style={{ border: "1.5px solid rgba(10,25,47,0.2)", color: "#0A192F" }} placeholder="/images/..." />
      </label>
      <label className="flex flex-col gap-1">
        <span className="text-xs font-semibold" style={{ color: "#6B7280" }}>Description</span>
        <textarea value={form.description} onChange={(e) => set("description", e.target.value)} rows={3} className="border rounded-xl px-3 py-2 text-sm outline-none resize-none" style={{ border: "1.5px solid rgba(10,25,47,0.2)", color: "#0A192F" }} />
      </label>
      <label className="flex items-center gap-2 cursor-pointer">
        <input type="checkbox" checked={form.published} onChange={(e) => set("published", e.target.checked)} className="w-4 h-4 rounded" />
        <span className="text-sm font-medium" style={{ color: "#0A192F" }}>Published on homepage</span>
      </label>
      <div className="flex gap-3">
        <button onClick={() => onSave(form)} className="px-5 py-2 rounded-xl text-sm font-semibold text-white" style={{ backgroundColor: "#0A192F" }}>Save Project</button>
  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "#0A192F" }}>Explore (Projects)</h1>
          <p className="text-sm mt-1" style={{ color: "#6B7280" }}>Manage town centre projects featured in the Explore section.</p>
        </div>
        <button onClick={() => setEditing({})} className="px-5 py-2 rounded-xl text-sm font-semibold text-white transition-opacity hover:opacity-90" style={{ backgroundColor: "#0A192F" }}>+ New Project</button>
      </div>
          ...
          <div key={p.id} className="bg-white rounded-2xl overflow-hidden flex gap-4" style={{ boxShadow: "0 2px 12px rgba(13,42,51,0.07)", border: "1px solid rgba(10,25,47,0.08)" }}>
            ...
            <div className="flex-1 min-w-0 p-4 flex flex-col gap-1 justify-center">
              <div className="flex items-center gap-2">
                <span className="font-bold text-sm" style={{ color: "#0A192F" }}>{p.title}</span>
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={p.published ? { backgroundColor: "rgba(10,25,47,0.12)", color: "#0A192F" } : { backgroundColor: "rgba(107,114,128,0.1)", color: "#6B7280" }}>
            ...
            <div className="flex flex-col items-end justify-center gap-2 px-4 shrink-0">
              <button onClick={() => setEditing(p)} className="px-4 py-1.5 rounded-lg text-xs font-semibold transition-opacity hover:opacity-70" style={{ border: "1.5px solid rgba(10,25,47,0.2)", color: "#0A192F" }}>Edit</button>
