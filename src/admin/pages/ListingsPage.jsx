...
export default function ListingsPage() {
  ...
  const columns = [
    ...
    { key: "tier", label: "Tier", render: (v) => (
      <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ backgroundColor: v === "Premium" ? "rgba(37,99,235,0.2)" : "rgba(16,24,40,0.08)", color: "#1E293B" }}>{v}</span>
    )},
  ...
  return (
    <div className="flex flex-col gap-6 max-w-6xl">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "#1E293B" }}>Listings (Business Directory)</h1>
          <p className="text-sm mt-1" style={{ color: "#6B7280" }}>All business directory entries across the portal.</p>
        </div>
        <button className="px-5 py-2 rounded-xl text-sm font-semibold text-white transition-opacity hover:opacity-90" style={{ backgroundColor: "#2563EB" }}>+ Add Listing</button>
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {SECTION_FILTERS.map((s) => (
          <button key={s} onClick={() => setSection(s)} className="px-4 py-1.5 rounded-full text-xs font-semibold transition-all"
            style={section === s ? { backgroundColor: "#2563EB", color: "#fff" } : { backgroundColor: "#fff", color: "#1E293B", border: "1.5px solid rgba(16,24,40,0.2)" }}>
            {s}
          </button>
        ))}
      </div>
