...
function Field({ label, value, mono }) {
  return (
    <div className="flex flex-col gap-0.5">
      ...
      <span className={`text-sm ${mono ? "font-mono" : "font-medium"}`} style={{ color: "#1E293B" }}>{value ?? "—"}</span>
    </div>
  );
}
        ...
        <>
          ...
          <Section title="Tags / attributes">
            <div className="flex gap-2 flex-wrap">
              {listing.tags.map((t) => (
                <span key={t} className="px-3 py-1 rounded-full text-xs font-semibold" style={{ backgroundColor: "rgba(16,24,40,0.08)", color: "#1E293B" }}>{t}</span>
              ))}
            </div>
          </Section>
        </>
          ...
          <Section title="Gallery images submitted">
            <div className="flex gap-3 flex-wrap">
              {listing.gallery.map((src, i) => (
                <img key={i} src={src} alt="" className="w-32 h-24 rounded-xl object-cover" style={{ border: "1.5px solid rgba(16,24,40,0.12)" }} />
              ))}
            </div>
          </Section>
        </>
  ...
  return (
    <>
      ...
      <Section title="Offer details">
        <div className="rounded-2xl p-5 flex flex-col gap-4" style={{ backgroundColor: "rgba(16,24,40,0.04)", border: "1.5px solid rgba(16,24,40,0.1)" }}>
          <div>
            <p className="text-lg font-bold" style={{ color: "#1E293B" }}>{offer.title}</p>
      ...
      {offer.image && (
        <>
          ...
          <Section title="Offer image">
            <img src={offer.image} alt="" className="w-48 h-36 rounded-xl object-cover" style={{ border: "1.5px solid rgba(16,24,40,0.12)" }} />
          </Section>
        </>
      )}
    </>
  ...
  return (
    <>
      ...
      <Section title={`Imported properties (${detail.properties.length})`}>
        <div className="rounded-xl overflow-hidden" style={{ border: "1px solid rgba(16,24,40,0.1)" }}>
            ...
            <thead>
              <tr style={{ backgroundColor: "rgba(16,24,40,0.05)", borderBottom: "1px solid rgba(16,24,40,0.1)" }}>
                {["Ref", "Address", "Type", "Beds", "Price", "Status"].map((h) => (
                  <th key={h} className="px-4 py-2.5 text-left text-[11px] font-bold uppercase tracking-wide" style={{ color: "#1E293B" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {detail.properties.map((p, i) => (
                <tr key={p.id} style={{ borderTop: i > 0 ? "1px solid rgba(16,24,40,0.07)" : "none" }}>
                  <td className="px-4 py-2.5 text-xs font-mono" style={{ color: "#9CA3AF" }}>{p.id}</td>
                  <td className="px-4 py-2.5 text-xs font-medium" style={{ color: "#1E293B" }}>{p.address}</td>
                  ...
                  <td className="px-4 py-2.5 text-xs font-semibold" style={{ color: "#1E293B" }}>{p.price}</td>
                  <td className="px-4 py-2.5">
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: p.status === "For Sale" ? "rgba(16,24,40,0.1)" : "rgba(37,99,235,0.1)", color: p.status === "For Sale" ? "#1E293B" : "#1D4ED8" }}>{p.status}</span>
                  </td>
                </tr>
              ))}
            </tbody>
  ...
  return (
    <div className="max-w-3xl flex flex-col gap-6">
      ...
      <div className="flex items-center gap-2 text-sm">
        <Link to="/admin/approvals" className="font-medium transition-opacity hover:opacity-70" style={{ color: "#1E293B" }}>← Approval Queue</Link>
      </div>

      {/* Header card */}
      <div className="bg-white rounded-2xl p-6 flex flex-col gap-4" style={{ boxShadow: "0 1px 2px rgba(16,24,40,0.04), 0 1px 3px rgba(16,24,40,0.06)", border: "1px solid rgba(16,24,40,0.08)" }}>
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl shrink-0" style={{ backgroundColor: "rgba(16,24,40,0.08)" }}>
              {TYPE_ICON[item.type] ?? "📄"}
            </div>
            <div>
              <h1 className="text-xl font-bold" style={{ color: "#1E293B" }}>{item.business}</h1>
        ...
        <div className="px-4 py-3 rounded-xl text-sm" style={{ backgroundColor: "rgba(16,24,40,0.04)", color: "#374151" }}>
          <span className="font-semibold" style={{ color: "#1E293B" }}>Summary: </span>{item.summary}
        </div>

        {/* Outcome message */}
        {message && (
          <div className="px-4 py-3 rounded-xl text-sm font-medium" style={{ backgroundColor: status === "Approved" ? "rgba(16,24,40,0.1)" : "rgba(185,28,28,0.07)", color: status === "Approved" ? "#1E293B" : "#991B1B" }}>
            {message}
          </div>
        )}
        ...
        {item.source !== "xml" && isPending && (
          <div className="flex flex-col gap-3 pt-1">
            {!rejecting ? (
              <div className="flex gap-3 flex-wrap">
                <button onClick={handleApprove} className="px-6 py-2.5 rounded-xl text-sm font-semibold text-white transition-opacity hover:opacity-90" style={{ backgroundColor: "#2563EB" }}>
                  ✓ Approve submission
                </button>
              ...
              <div className="flex flex-col gap-2">
                ...
                <textarea
                  ...
                  style={{ border: "1.5px solid rgba(185,28,28,0.35)", color: "#1E293B", backgroundColor: "#fff" }}
                />
      ...
      {/* Content detail card */}
      <div className="bg-white rounded-2xl p-6 flex flex-col gap-6" style={{ boxShadow: "0 1px 2px rgba(16,24,40,0.04), 0 1px 3px rgba(16,24,40,0.06)", border: "1px solid rgba(16,24,40,0.08)" }}>
        <h2 className="font-bold text-base" style={{ color: "#1E293B" }}>Submission Content</h2>
