...
function Toast({ message, onDismiss }) {
  ...
  return (
    <div
      ...
      style={{ backgroundColor: "#0A192F", color: "#fff" }}
    >
  ...
  return (
    <div className="bg-white rounded-2xl p-6 flex flex-col gap-5" style={{ boxShadow: "inset 0 1px 0 rgba(255,255,255,0.95), inset 0 -2px 5px rgba(10,25,47,0.035), 0 1px 2px rgba(10,25,47,0.06), 0 14px 30px -12px rgba(10,25,47,0.28)", border: "1.5px solid rgba(10,25,47,0.12)" }}>
      ...
      <div className="flex gap-3 pt-2 border-t" style={{ borderColor: "rgba(10,25,47,0.1)" }}>
        <button onClick={() => onSave(form)} disabled={!isValid} className="px-6 py-2.5 rounded-xl text-sm font-semibold text-white transition-opacity disabled:opacity-40" style={{ backgroundColor: "#0A192F" }}>
          Register Business
        </button>
    ...
    <div className="bg-white rounded-2xl p-5 flex items-start gap-4 flex-wrap" style={{ boxShadow: "inset 0 1px 0 rgba(255,255,255,0.95), inset 0 -2px 5px rgba(10,25,47,0.035), 0 1px 2px rgba(10,25,47,0.06), 0 14px 30px -12px rgba(10,25,47,0.28)", border: "1px solid rgba(10,25,47,0.08)" }}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap mb-1">
          <span className="text-sm font-bold" style={{ color: "#0A192F" }}>{biz.name}</span>
          <StatusTag status={biz.status} />
          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide" style={{ backgroundColor: "rgba(10,25,47,0.07)", color: "#6B7280" }}>{biz.plan}</span>
        </div>
        <p className="text-xs font-semibold mb-1" style={{ color: "#0A192F" }}>{biz.category}</p>
      ...
      <div className="flex flex-col gap-2 shrink-0">
        {biz.status === "Pending" ? (
          <>
            <button onClick={() => onApprove(biz)} className="px-4 py-2 rounded-xl text-xs font-semibold text-white transition-opacity hover:opacity-90" style={{ backgroundColor: "#0A192F" }}>✓ Approve Users</button>
          ...
          <span className="text-xs font-semibold px-3 py-2 rounded-xl text-center" style={{ backgroundColor: "rgba(10,25,47,0.1)", color: "#0A192F" }}>✓ Users active</span>
        ) : (
          <button onClick={() => onApprove(biz)} className="px-4 py-2 rounded-xl text-xs font-semibold transition-opacity hover:opacity-70" style={{ border: "1.5px solid rgba(10,25,47,0.2)", color: "#0A192F" }}>Re-approve</button>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "#0A192F" }}>Business Registrations</h1>
        ...
        {!showForm && (
          <button onClick={() => setShowForm(true)} className="px-5 py-2.5 rounded-xl text-sm font-semibold text-white transition-opacity hover:opacity-90 shrink-0" style={{ backgroundColor: "#0A192F" }}>
            + Register Business
          </button>
        )}
      </div>
