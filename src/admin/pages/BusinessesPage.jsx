...
function RegisterBusinessForm({ onSave, onCancel }) {
  ...
  const field = { border: "1.5px solid rgba(16,24,40,0.2)", color: "#1E293B" };
  ...
  return (
    <div className="bg-white rounded-2xl p-6 flex flex-col gap-5" style={{ boxShadow: "0 1px 2px rgba(16,24,40,0.04), 0 1px 3px rgba(16,24,40,0.06)", border: "1.5px solid rgba(16,24,40,0.12)" }}>
      <div className="flex items-center justify-between gap-4">
        <h3 className="text-base font-bold" style={{ color: "#1E293B" }}>Register New Business</h3>
      ...
      <div className="flex gap-3 pt-2 border-t" style={{ borderColor: "rgba(16,24,40,0.1)" }}>
        <button onClick={() => onSave(form)} disabled={!isValid} className="px-6 py-2.5 rounded-xl text-sm font-semibold text-white transition-opacity disabled:opacity-40" style={{ backgroundColor: "#2563EB" }}>
          Register Business
        </button>
...
function BusinessRow({ biz, onApprove, onReject }) {
  return (
    <div className="bg-white rounded-2xl p-5 flex items-start gap-4 flex-wrap" style={{ boxShadow: "0 1px 2px rgba(16,24,40,0.04), 0 1px 3px rgba(16,24,40,0.06)", border: "1px solid rgba(16,24,40,0.08)" }}>
      <div className="w-11 h-11 rounded-xl flex items-center justify-center text-base font-bold shrink-0" style={{ backgroundColor: "rgba(16,24,40,0.1)", color: "#1E293B" }}>
        {biz.name[0]}
      </div>

      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap mb-1">
          <span className="text-sm font-bold" style={{ color: "#1E293B" }}>{biz.name}</span>
          <StatusTag status={biz.status} />
          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide" style={{ backgroundColor: "rgba(16,24,40,0.07)", color: "#6B7280" }}>{biz.plan}</span>
        </div>
        <p className="text-xs font-semibold mb-1" style={{ color: "#1E293B" }}>{biz.category}</p>
      ...
      <div className="flex flex-col gap-2 shrink-0">
        {biz.status === "Pending" ? (
          <>
            <button onClick={() => onApprove(biz)} className="px-4 py-2 rounded-xl text-xs font-semibold text-white transition-opacity hover:opacity-90" style={{ backgroundColor: "#2563EB" }}>✓ Approve Users</button>
          ...
          <span className="text-xs font-semibold px-3 py-2 rounded-xl text-center" style={{ backgroundColor: "rgba(16,24,40,0.1)", color: "#1E293B" }}>✓ Users active</span>
        ) : (
          <button onClick={() => onApprove(biz)} className="px-4 py-2 rounded-xl text-xs font-semibold transition-opacity hover:opacity-70" style={{ border: "1.5px solid rgba(16,24,40,0.2)", color: "#1E293B" }}>Re-approve</button>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "#1E293B" }}>Business Registrations</h1>
        ...
        {!showForm && (
          <button onClick={() => setShowForm(true)} className="px-5 py-2.5 rounded-xl text-sm font-semibold text-white transition-opacity hover:opacity-90 shrink-0" style={{ backgroundColor: "#2563EB" }}>
            + Register Business
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: "Approved", value: counts.Approved, accent: "#1E293B" },
          { label: "Rejected", value: counts.Rejected, accent: "#991B1B" },
        ].map(({ label, value, accent }) => (
          <div key={label} className="bg-white rounded-2xl p-4 flex flex-col gap-1" style={{ boxShadow: "0 1px 2px rgba(16,24,40,0.04), 0 1px 3px rgba(16,24,40,0.06)", border: "1px solid rgba(16,24,40,0.08)" }}>
