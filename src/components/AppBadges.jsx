...
//
// Badges sit side-by-side: on narrow containers / mobile they flex to share the
// row equally; on wider containers they take their natural width.
...
function Badge({ href, icon, top, bottom, sm }) {
  return (
    <a
      href={href}
...
export default function AppBadges({ className = "", size = "default" }) {
  const sm = size === "sm";
  const iconSize = sm ? { apple: 18, gp: 16 } : { apple: 22, gp: 20 };
  return (
    <div className={`flex flex-nowrap gap-3 ${className}`}>
      <Badge href={APP_STORE_URL} icon={<AppleIcon size={iconSize.apple} />} top="Download on the" bottom="App Store" sm={sm} />
      <Badge href={PLAY_STORE_URL} icon={<GooglePlayIcon size={iconSize.gp} />} top="GET IT ON" bottom="Google Play" sm={sm} />
    </div>
  );
}
